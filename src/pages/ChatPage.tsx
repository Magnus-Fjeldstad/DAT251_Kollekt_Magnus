    ...
    return (
      <div className="flex flex-col h-[calc(100vh-8.5rem)] pt-4 animate-pulse space-y-3">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className={`glass rounded-2xl h-12 ${i % 2 === 0 ? "w-2/3" : "w-1/2 ml-auto"}`}
          />
        ))}
      </div>
    );
    ...
    >
            ...
            >
              <div className="max-w-[80%] space-y-1">
                {replyTarget && (
                  ...
                  >
                    <div
                      className={`mb-1 flex items-center gap-1 ${isSelf ? "text-primary/80" : "text-muted-foreground"}`}
                    >
                      <Reply className="h-2.5 w-2.5" />
                      <span>
                        {t("chat.replyingTo", { name: replyTarget.sender })}
                      </span>
                    </div>
                    <p
                      className={`font-semibold ${isSelf ? "text-foreground" : "text-primary"}`}
                    >
                      {replyTarget.sender}
                    </p>
                    <p className="truncate">
                      {replyTarget.text || t("chat.imageAlt")}
                    </p>
                  </div>
                )}
                <div
                ...
                >
                  ...
                  <p
                    className={`text-[9px] mt-1 ${isSelf ? "text-primary-foreground/60" : "text-muted-foreground"}`}
                  >
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setReactingId(
                        reactingId === message.id ? null : message.id,
                      )
                    }
                    className="h-6 w-6 rounded-full glass flex items-center justify-center"
                ...
                {message.reactions.length > 0 && (
                  <div
                    className={`px-1 flex gap-1 flex-wrap ${isSelf ? "justify-end" : "justify-start"}`}
                  >
          ...
          >
            <div className="glass rounded-xl p-3 space-y-2 mb-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={sendPoll}
                  className="ml-auto px-3 py-1 rounded-lg gradient-primary text-[10px] font-semibold text-primary-foreground"
                >
                  {t("chat.sendPoll")}
                </button>
              </div>
        ...
        <div className="glass rounded-lg px-3 py-2 mb-2 flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="text-[10px] font-semibold text-primary">
              {t("chat.replyingTo", {
                name: messageById.get(replyingToId)?.sender,
              })}
            </p>
            <p className="text-xs truncate text-muted-foreground">
              {messageById.get(replyingToId)?.text || t("chat.imageAlt")}
            </p>
          </div>
          <button
      ...
      <div className="flex gap-2 pt-2 pb-1">
        ...
        <button
          ...
          className="h-10 w-10 rounded-xl glass flex items-center justify-center shrink-0"
          aria-label={t("chat.togglePollForm")}
        >
        ...
        <button
          onClick={sendMessage}
          className="h-10 w-10 rounded-xl gradient-primary flex items-center justify-center shrink-0"
